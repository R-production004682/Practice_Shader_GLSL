...
int main(int argc, char** argv)
{
    ...
    glutIdleFunc(idle);
    init();
    initGlsl(&shaderProgram, "Gouraud.vert");
    glutMainLoop();
    return 0;
}
...
void display(void) {
    ...
    glShadeModel(flagFlat ? GL_FLAT : GL_SMOOTH);
...
void special(int key, int x, int y) {
    const float step = 5.0;  // カメラ回転用ステップ
    const float lightStep = 0.5; // 光源移動用ステップ

    if (flagLight) {
        // 光源移動モード
        switch (key) {
        case GLUT_KEY_UP:    // 上矢印キーで光源を上に移動
            lightPosition[1] += lightStep;
            break;
        case GLUT_KEY_DOWN:  // 下矢印キーで光源を下に移動
            lightPosition[1] -= lightStep;
            break;
        case GLUT_KEY_LEFT:  // 左矢印キーで光源を左に移動
            lightPosition[0] -= lightStep;
            break;
        case GLUT_KEY_RIGHT: // 右矢印キーで光源を右に移動
            lightPosition[0] += lightStep;
            break;
        default:
            break;
...
void mouse(int button, int state, int x, int y) {
    if (state == GLUT_DOWN) {
        ...
        tStart = y;
        flagMouse = true;
        sKey = (button == GLUT_LEFT_BUTTON) ? ROTATE : (button == GLUT_RIGHT_BUTTON ? TRANSLATE : SCALE);
    }
